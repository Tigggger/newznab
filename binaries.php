...
{  
	...
	function scan($nntp, $groupArr, $first, $last, $type='update')
	{
		...
		{	
			...
			if ($type != 'partrepair')
				echo "Received ".number_format(sizeof($msgsreceived))." of ".number_format(($last-$first+1))." requested, ".number_format(sizeof($msgsblacklisted))." blacklisted, ".number_format(sizeof($msgsignored))." not binaries $n";
    ...
    private function partRepair($nntp, $group)
    {
        ...
        if (count($parts))
        {
            ...
            {
                if (in_array(substr($item, 0, -3), $matched))
                    continue;
                    
                # when moving to php >= 5.3
                #preg_filter(sprintf("~%s~", substr($item, 0, -3)), '$0', $parts);
                
                $result = preg_grep(sprintf("~%s~", substr($item, 0, -3)), $parts);
                if (count($result))
                {
                    $matched[] = substr($item, 0, -3);
            ...
            if (!count($chunks))
            {
                printf("Repair: unable to extract parts for repair! Please report this is a bug.\n", $this->n);
                return false;
            }
