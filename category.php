  ...
  const CAT_GAME_NDS = 1010;
    ...
    */
	...
	function determineCategory($group, $releasename = "")
	{
		...
		//
			...
			if($this->isMusic($releasename)){ return $this->tmpCat; }
			if($this->isEBook($releasename)){ return $this->tmpCat; }
			return Category::CAT_NOT_DETERMINED;
	}
	...
	*/
	...
	public function isTV($releasename)
	{
		...
		else if (preg_match('/( S\d{1,2} |\.S\d{2}\.|\.S\d{2}|s\d{1,2}e\d{1,2}|(\.| |\b|\-)EP\d{1,2}\.|\.E\d{1,2}\.|special.*?HDTV|HDTV.*?special|History( |\.|\-)Channel|trollhd|trollsd|webrip|HDTV.*?BTL|C4TV|WEB DL|web\.dl|WWE|season \d{1,2}|(?!collectors).*?series|\.TV\.|\.dtv\.|UFC|TNA|staffel|episode|special\.\d{4})/i', $releasename))
		{
			//echo "tv2";
			if($this->isForeignTV($releasename)){ return true; }
			if($this->isSportTV($releasename)){ return true; }
	...
	public function isForeignTV($releasename)
	{
		...
		else if(preg_match('/\.des\.(?!moines)|Chinese\.Subbed|vostfr|Hebrew\.Dubbed|\.HEB\.|Hebdub|NLSubs|NL\-Subs|NLSub|Deutsch| der |German | NL |staffel|videomann/i', $releasename))
		{
			$this->tmpCat = Category::CAT_TV_FOREIGN;
			return true;
		}
	...
	public function isSportTV($releasename)
	{
		...
		else if(preg_match('/Twenty20|wimbledon|Kentucky\.Derby|WBA|Rugby\.|TNA\.|DTM\.|NASCAR|SBK|NBA(\.| )|NHL\.|NRL\.|MLB\.|Playoffs|FIFA\.|Serie.A|netball\.anz|formula1|indycar|Superleague|V8\.Supercars|((19|20)\d{2}.*?olympics?|olympics?.*?(19|20)\d{2})|x(\ |\.|\-)games/i', $releasename))
		{	
			$this->tmpCat = Category::CAT_TV_SPORT;
			return true;
		}
	...
	*/	
	...
	{
		if (preg_match('/dvdrip|xvid|x264/i', $releasename))
		{
			return false;
		}
	...
	public function isEBook($releasename)
	{
		...
		if (preg_match('/Ebook|E?\-book|\) WW|\[Springer\]|[ .\/\\-](epub|mobi|pdf)|eMag|ISBN/i', $releasename))
		{
