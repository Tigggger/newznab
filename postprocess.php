...
class PostProcess 
{
	...
	{		
		if ($this->site->lookupnfo == 1)
		{
			...
			// Because not everyone is ready to have their entire library
			// scanned, we process them all in large batches... eventually
			// after each release we'll catch up and handle everything
			$batch = 500;
			$nfo->processNfoFiles($batch);
		}
	}
	...
	public function processAdditional()
	{
		...
		//
		$data = $db->query(sprintf("select COUNT(*) AS ToDo from releases r left join category c on c.ID = r.categoryID where (r.passwordstatus between -6 and -1) or (r.haspreview = -1 and c.disablepreview = 0)"));
		$total = $data[0]['ToDo'];
		...
		if ($rescount > 0)
		{
			echo "PostPrc : Performing additional post processing on last ".$rescount."/".$total." releases ...\n";		
			$nntpconnected = false;
			
			foreach ($result as $rel)
			{
				echo "PostPrc: ".$rel['name']."\n";
    ...
    public function lameAudioSample($lameinfo, $releaseguid)
    {
        ...
        if (file_exists($samplefile))
        {
            ...
            $output = runCmd($execstring, false, true);        
            $outputogg = runCmd($execstringogg, false, true);        
